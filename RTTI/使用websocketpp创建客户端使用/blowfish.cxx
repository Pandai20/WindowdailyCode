...
CBlowFish::CBlowFish (const uint8_t *key, int keybytes)
{
    PArray = new uint32_t[NPASS + 2];
...
void CBlowFish::Initialize(const uint8_t *key, int keybytes)
{
    int  		i, j;
...
uint32_t CBlowFish::GetOutputLength(uint32_t lInputLong, int mode)
{
    if (mode & BF_PKCS5)
        lInputLong++;
...
uint32_t CBlowFish::Encrypt(const uint8_t *pInput, uint8_t *pOutput, uint32_t lSize, int mode, int64_t IV)
{
    ...
    for (lCount = 0; lCount < lOutSize; lCount += BF_BLKSIZE) {
        ...
        change_order(dwOutput);
        change_order(dwOutput + 1);
...
// output buffer can be the same, but be sure buffer length is even MOD 8.
uint32_t CBlowFish::Decrypt(const uint8_t *pInput, uint8_t *pOutput, uint32_t lSize, int mode, int64_t IV)
{
